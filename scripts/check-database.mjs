...
assert.equal(payload.ref, "pmpekvgfhlixpivwfyfs");
...
if(process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY){
  ...
  for(const table of ['profiles','purchases']) {const {error,data}=await anon.from(table).select('*').limit(1);assert.ok(error&&!data,`${table} must reject direct anonymous reads`);}
